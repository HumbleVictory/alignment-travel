...
type SetupWeights = {
  ...
  nightlife: number;
  comfort: number;
  cost: number; // cost sensitivity

  // ✅ required by ResultsClient mapping
  safety: number;
  shopping: number;
  weather: number;
  crowds: number;
...
export default function SetupPage() {
  ...
  const [weights, setWeights] = useState<SetupWeights>({
    ...
    // ✅ defaults (so ResultsClient always has these)
    safety: 50,
    shopping: 50,
    weather: 50,
    crowds: 50,
  ...
  useEffect(() => {
    ...
    setWeights({
      ...
      // ✅ load stored values (or fall back)
  ...
  useEffect(() => {
    ...
    // ✅ NEW: directly map these from profile weights (fallback to 50)
        ...
        <div className="shell p-6 md:p-8">
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-[340px_1fr]">
            <section className="panel p-5">
              <div className="text-sm font-semibold text-white/85">Basics</div>

              <div className="mt-5 space-y-4">
                <div>
                  <div className="text-xs font-medium text-white/55">Traveler profile</div>
                  <select
                    value={profileId}
                ...
                <div>
                  ...
                  <input
                    ...
                    min={0}
                    value={budgetUsd}
                    onChange={(e) => setBudgetUsd(clamp(parseInt(e.target.value || "0", 10), 0, 1000000))}
                ...
                <Link
                  href="/results"
                  className="ui-btn mt-2 inline-flex w-full items-center justify-center rounded-xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-emerald-950 hover:bg-emerald-400"
                >
                  Continue to results
                </Link>
            ...
            <section className="panel p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-sm font-semibold text-white/85">Priorities</div>
                ...
                <div className="text-xs text-white/45">Range: 0–100</div>
              </div>
              ...
              <div className="mt-5 space-y-4">
                {(
                  [
                    ...
                    // ✅ NEW (same UI cards, same layout style)
                  ...
                  return (
                    <div key={key} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                      ...
                      {key === "crowds" ? (
                        <div className="mt-1 text-[11px] text-white/60">
                          Higher = you care more about avoiding crowds (low-crowd cities score higher)
                        </div>
                      ) : null}
                      ...
                      <div className="mt-2 text-[11px] text-white/50">
                        {isCustom ? "Higher = matters more" : "Locked by profile (switch to Custom to edit)"}
                      </div>
                    </div>
                  );
                })}
